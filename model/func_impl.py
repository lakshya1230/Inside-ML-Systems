...
def get_info(
    ...
    elif fc_layer in ['fc_q', 'fc_k', 'fc_v']:
        part_in_dim = in_dim
        part_out_dim = out_dim // mp_size

    mp_comm = comm.Split(dp_idx, mp_idx)
    dp_comm = comm.Split(mp_idx, dp_idx)
...
def naive_collect_forward_input(
    ...
    part_in_dim = x.shape[2]

    in_dim = part_in_dim * mp_size

    collected_x = np.empty(batch_size * seq_length * in_dim, dtype=np.float64)

    mp_comm.Allgather(x.flatten(), collected_x)
    collected_x = collected_x.reshape(mp_size, batch_size, seq_length, part_in_dim)
    collected_x = collected_x.transpose(1,2,0,3).reshape(batch_size, seq_length, in_dim)
    return collected_x
...
def naive_collect_forward_output(
    ...
    part_out_dim = out.shape[2]

    out_dim = part_out_dim * mp_size

    collected_out = np.empty(batch_size * seq_length * out_dim, dtype=np.float64)

    mp_comm.Allgather(out.flatten(), collected_out)
    collected_out = collected_out.reshape(mp_size, batch_size, seq_length, part_out_dim)
    collected_out = collected_out.transpose(1,2,0,3).reshape(batch_size, seq_length, out_dim)
...
def naive_collect_backward_output(
    ...
    st = mp_group_idx * part_out_dim
    end = (mp_group_idx + 1) * part_out_dim
    return output_grad[:,:, st:end]
...
def naive_collect_backward_x(
    ...
    seq_length = grad_x.shape[1]
    in_dim = grad_x.shape[2]
    ...
    collected_grad_x = np.empty(batch_size * seq_length * part_in_dim, dtype=np.float64)

    mp_comm.Reduce_scatter(grad_x.flatten(), collected_grad_x, op = MPI.SUM)

    collected_grad_x = collected_grad_x.reshape(batch_size, seq_length, part_in_dim)
