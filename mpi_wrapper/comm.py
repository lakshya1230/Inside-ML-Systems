...
class Communicator(object):
    ...
    def myAlltoall(self, src_array, dest_array):
        ...
        src_segment = src_array.size // nprocs
        des_segment = dest_array.size // nprocs

        send_bytes = src_array.itemsize * src_segment
        rec_bytes  = dest_array.itemsize * des_segment

       

        for i in range(0, nprocs):
            if(i != rank):
                self.total_bytes_transferred += send_bytes
                self.comm.Send([src_array[ src_segment * i : src_segment * (i + 1)], MPI.INT], dest=i)


        for i in range(0, nprocs):
            if(i != rank):
                self.comm.Recv([dest_array[ des_segment * i : des_segment * (i + 1)], MPI.INT], source=i)
                self.total_bytes_transferred += rec_bytes
            else:
                dest_array[ des_segment * i :  des_segment * (i + 1) ] = src_array[src_segment * i: src_segment * (i + 1) ]
